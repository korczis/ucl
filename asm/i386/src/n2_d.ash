...
%macro decode_check_off 1
  %ifdef UCL_SAFE
                js      near L_lookbehind_overrun   ; error if >= 0x80000000
  %endif
%endmacro

%macro decode_check_len 1
  %ifdef UCL_SAFE
                js      near L_output_overrun       ; error if >= 0x80000000
  %endif
%endmacro
...
%macro copy_match.init 2
  %ifdef UCL_SAFE
                ...
                mov     %1, edi
                add     %1, ecx
                jc      near L_output_overrun
                cmp     %1, OUT_END
                ja      near L_output_overrun
                ; error if [edi+%2] is < OUT
                mov     %1, edi
                add     %1, %2
                jnc     near L_lookbehind_overrun
                cmp     %1, OUT
                jb      near L_lookbehind_overrun
  %else
                lea     %1, [edi+%2]
  %endif
%endmacro
...
%ifdef UCL_SAFE
                ; must be negative
                jns     near L_lookbehind_overrun
%endif
                sar     eax, 1                  ; shift low-bit into carry
                mov     ebp, eax
                jmps    .decompr_got_off
